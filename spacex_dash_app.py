...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'All':
        success_df = filtered_df.groupby('Launch Site')['class'].mean().reset_index()
        ...
        success_df = filtered_df.groupby('class')['Launch Site'].count().reset_index()
        success_df["names"] = ["Success","Failure"]
        success_df = success_df.rename(columns={'Launch Site': 'Count'})
        print(success_df)
        fig = px.pie(success_df, values='Count', 
            names='names',  
